...
function renderTable(cards) {
  ...
  document.getElementById('cards-tbody').innerHTML = cards.map(c => {
    ...
    const rowClass = isCc ? 'row-cc-card' : (c.status === 'modified' ? 'row-modified' : '');
    const ccBadge  = isCc
      ? `<span style="display:inline-block;background:linear-gradient(135deg,#7b68ee,#5f4fcf);color:#fff;font-size:9px;font-weight:700;padding:1px 7px;border-radius:12px;margin-right:4px;vertical-align:middle">📞CC</span>`
      : '';
    return `
    <tr class="${rowClass}">
      <td>${ccBadge}<span class="ac-num">#${c.account_number}</span></td>
