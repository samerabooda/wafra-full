...
async function loadFormOptions() {
  ...
  if (employees.success) {
    ...
    ['f-broker','f-marketer','f-ext1','f-ext2'].forEach(id => {
      ...
      const isBroker = id === 'f-broker';
      // Always add a blank first option so nothing is silently pre-selected
      const blank = document.createElement('option');
      blank.value = '';
      blank.textContent = isBroker ? '— اختر البروكر —' : '— لا يوجد —';
...
async function submitCard() {
  ...
  const payload = {
    account_number:     ac,
    month:              month,
    // month_date is derived server-side from month (e.g. "Jan 2025" → "2025-01-01")
