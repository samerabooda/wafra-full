    ...
    if (br.success) {
      bEl.innerHTML = br.data.map(b =>
        `<div style="display:flex;align-items:center;gap:8px;padding:8px 0;border-bottom:1px solid var(--brd1)">
          ...
          ${b.commission_cards_count > 0 ? `<span style="font-size:10px;color:var(--mu)">${b.commission_cards_count} حساب</span>` : ''}
          <button class="btn btn-ghost btn-sm" style="color:var(--re)" onclick="deleteBranch(${b.id},'${b.name_ar}',${b.commission_cards_count||0})">✕</button>
        </div>`
...
async function deleteBranch(id, nameAr, cardCount) {
  const warn = cardCount > 0
    ? `⚠️ هذا الفرع يحتوي على ${cardCount} حساب.\nبعد الحذف ستصبح هذه الحسابات ضمن «حسابات بدون فرع».\n\nهل تريد المتابعة؟`
    : `حذف الفرع "${nameAr}"؟`;
  if (!confirm(warn)) return;
  const r = await api('DELETE', `/branches/${id}`);
  if (r.success) { toast(r.message || 'تم الحذف ✅', 'success'); loadSettings(); }
  else toast(r.message, 'error');
}
